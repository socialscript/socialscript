/*
 * JQuery Form Builder - Radio plugin.
 *
 * Revision: @REVISION
 * Version: @VERSION
...
var FbRadio = $.extend({}, $.fb.fbWidget.prototype, {
	...
	_getWidget : function(event, fb) {
		...
		fb.target._log('fbRadio._getWidget executing...');
		...
		fb.target._log('fbRadio._getWidget executed.');
		return $jqueryObject;
	},
	_getFieldSettingsLanguageSection : function(event, fb) {
		fb.target._log('fbRadio._getFieldSettingsLanguageSection executing...');
    ...
    $('input', $label).val(fb.settings.label)
     .keyup(function(event) {
	  ...
	  var $value = fb.target._label({ label: 'Name 1', name: 'field.value' })
		                      .append('<input type="text" id="field.value" />')
		                      ._label({ label: 'Value 1', name: 'field.value' })
		                      .append('<input type="text" id="field.value" />')
		                      ._label({ label: 'Name 2', name: 'field.value' })
		                      .append('<input type="text" id="field.value" />')
		                       ._label({ label: 'Value 2', name: 'field.value' })
		                      .append('<input type="text" id="field.value" />')
		                      ;
    ...
    var fontFamily = styles.fontFamily != 'default' ? styles.fontFamily : fbStyles.fontFamily ;
	  var fontSize = styles.fontSize != 'default' ? styles.fontSize : fbStyles.fontSize;
		...
		fb.target._log('fbRadio._getFieldSettingsLanguageSection executed.');
	...
	_getFieldSettingsGeneralSection : function(event, fb) {
		fb.target._log('fbRadio._getFieldSettingsGeneralSection executing...');
		...
		fb.target._log('fbRadio._getFieldSettingsGeneralSection executed.');
		return [$valuePanel, $colorPanel];
	},
	_languageChange : function(event, fb) {
		fb.target._log('fbRadio.languageChange executing...');
		var styles = fb.settings.styles;
		var fbStyles = fb.target._getFbLocalizedSettings().styles;
		var fontFamily = styles.fontFamily != 'default' ? styles.fontFamily : fbStyles.fontFamily;
		var fontSize = styles.fontSize != 'default' ? styles.fontSize : fbStyles.fontSize;
    ...
    fb.item.css('fontFamily', fontFamily);
		...
		fb.target._log('fbRadio.languageChange executed.');
	}
});
