/*
 * JQuery Form Builder - Email Address plugin.
 *
 * Revision: @REVISION
 * Version: @VERSION
...
var FbEmailAddress = $.extend({}, $.fb.fbWidget.prototype, {
	...
	_getWidget : function(event, fb) {
		...
		fb.target._log('fbEmailAddress._getWidget executing...');
		...
		fb.target._log('fbEmailAddress._getWidget executed.');
		return $jqueryObject;
	},
	_getFieldSettingsLanguageSection : function(event, fb) {
		fb.target._log('fbEmailAddress._getFieldSettingsLanguageSection executing...');
    ...
    var fontFamily = styles.fontFamily != 'default' ? styles.fontFamily : fbStyles.fontFamily ;
	  var fontSize = styles.fontSize != 'default' ? styles.fontSize : fbStyles.fontSize;
		...
		fb.target._log('fbEmailAddress._getFieldSettingsLanguageSection executed.');
	...
	_getFieldSettingsGeneralSection : function(event, fb) {
		fb.target._log('fbEmailAddress._getFieldSettingsGeneralSection executing...');
		...
		fb.target._log('fbEmailAddress._getFieldSettingsGeneralSection executed.');
		return [$valuePanel, $colorPanel];
	},
	_languageChange : function(event, fb) {
		fb.target._log('fbEmailAddress.languageChange executing...');
		var styles = fb.settings.styles;
		var fbStyles = fb.target._getFbLocalizedSettings().styles;
		var fontFamily = styles.fontFamily != 'default' ? styles.fontFamily : fbStyles.fontFamily;
		var fontSize = styles.fontSize != 'default' ? styles.fontSize : fbStyles.fontSize;
    ...
    fb.item.css('fontFamily', fontFamily);
		...
		fb.target._log('fbEmailAddress.languageChange executed.');
	}
});

$.widget('fb.fbEmailAddress', FbEmailAddress);
