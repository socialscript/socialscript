...
$.Window = (function()  {
	...
	var OPACITY_MINIMIZED = 0.6;    // css opacity while window minimized or doing animation
	...
	var setting = {
		...
		animationSpeed:100,            // [number:400] the speed of animations: maximize, minimize, restore, shift, in milliseconds
	...
	function getCssStyleByDock(parent, miniIndex) {
		...
		if( setting.dock == 'left' || setting.dock == 'right' ) {
			...
			if( setting.dock == 'top' ) {
				...
				if( parent != null ) {
					targetCss.top = cpos.top + parent.height() - narrow - 2 + 10;
					targetCss.left += cpos.left;
				} else {
	...
	function constructor(caller, options) {
		...
		var options = $.extend({
			...
			z: 100,                      // [number:2000] the css z-index value
	...
	return { // static public methods
		...
		minimizeAll: function() { // hide all created windows
			for( var i=0, len=windowStorage.length; i<len; i++ ) {
				...
				//windowStorage[i].getContainer().hide();
				winid = windowStorage[i].getWindowId();
				win = $.window.getWindow(winid);
