...
// this script relies on the superglobal arrays, fake it here for old PHP
// versions
if(phpversion() < '4.1.0') {
...
function PasswordStrength($password) {
	$strength = 0;
	$strength += strlen(preg_replace('#[^a-z]#', '', $password)) * 0.5; // lowercase
	                                                                          // characters
	                                                                          // are
	                                                                          // weak
	$strength += strlen(preg_replace('#[^A-Z]#', '', $password)) * 0.8; // uppercase
	                                                                          // characters
	                                                                          // are
	                                                                          // somewhat
	                                                                          // better
	$strength += strlen(preg_replace('#[^0-9]#', '', $password)) * 1.0; // numbers
	                                                                          // are
	                                                                          // somewhat
	                                                                          // better
	$strength += strlen(preg_replace('#[a-zA-Z0-9]#', '', $password)) * 2.0; // other
	                                                                          // non-alphanumeric
	                                                                          // characters
	                                                                          // are
	                                                                          // best
...
ob_start();
if(! include_once ('../application/lib/PhpThumb/phpthumb.class.php')) {
	ob_end_flush();
	die('failed to include_once("' . realpath('../application/lib/PhpThumb/phpthumb.class.php') . '")');
}
ob_end_clean();
...
if(! phpthumb_functions::FunctionIsDisabled('set_time_limit')) {
	set_time_limit(60); // shouldn't take nearly this long in most cases, but
	                    // with many filters and/or a slow server...
}

// phpThumbDebug[0] used to be here, but may reveal too much
// info when high_security_mode should be enabled (not set yet)

if(file_exists('../application/lib/PhpThumb/phpThumb.config.php')) {
	ob_start();
	if(include_once ('../application/lib/PhpThumb/phpThumb.config.php')) {
		// great
	} else {
		ob_end_flush();
		$phpThumb->config_disable_debug = false; // otherwise error message won't
		                                         // print
		$phpThumb->ErrorImage('failed to include_once(' . '../application/lib/PhpThumb/phpThumb.config.php) - realpath="' . realpath('../application/lib/PhpThumb/phpThumb.config.php') . '"');
	}
	ob_end_clean();
} elseif(file_exists('../application/lib/PhpThumb/phpThumb.config.php.default')) {
	$phpThumb->config_disable_debug = false; // otherwise error message won't
	                                         // print
...
if(get_magic_quotes_gpc()) {
	...
	$RequestVarsToStripSlashes = array(
			'src',
			'wmf',
			'down'
	);
...
// if (!@$_GET['phpThumbDebug'] && !is_file($phpThumb->sourceFilename) &&
...
while ( $CanPassThroughDirectly && $phpThumb->src ) {
	...
	/*
	 * if (preg_match('#^http\://[^\\?&]+\.(jpe?g|gif|png)$#i', $phpThumb->src))
	 * { $phpThumb->DebugMessage('Passing HTTP source through directly as
	 * Location: redirect ('.$phpThumb->src.')', __FILE__, __LINE__);
	 * header('Location: '.$phpThumb->src); exit; }
	 */
	...
	if($phpThumb->getimagesizeinfo = @GetImageSize($SourceFilename)) {
		...
		if(! @$_GET['w'] && ! @$_GET['wp'] && ! @$_GET['wl'] && ! @$_GET['ws'] && ! @$_GET['h'] && ! @$_GET['hp'] && ! @$_GET['hl'] && ! @$_GET['hs']) {
			...
			// image fits into 'w'x'h' box, and at least one dimension matches
			// exactly, therefore no resizing needed
		...
		switch ($phpThumb->getimagesizeinfo[2]) {
			...
			case 3 : // PNG
				...
				break;
			default :
				// browser probably can't handle format, remangle it to
				// JPEG/PNG/GIF
