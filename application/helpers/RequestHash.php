...
class RequestHash {
	static function Generate() {
		return md5($_SERVER['REMOTE_ADDR'] . $_SERVER['SERVER_ADDR'] . $_SERVER['HTTP_USER_AGENT'] . $_SERVER['SERVER_ADMIN'] . $_SERVER['SERVER_SIGNATURE']);
	}
	static function validateHash($Hash) {
		if(md5($_SERVER['REMOTE_ADDR'] . $_SERVER['SERVER_ADDR'] . $_SERVER['HTTP_USER_AGENT'] . $_SERVER['SERVER_ADMIN'] . $_SERVER['SERVER_SIGNATURE']) == $Hash) {
			return TRUE;
		} else {
			return FALSE;
		}
