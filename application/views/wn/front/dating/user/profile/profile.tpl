...
<div class="left_profile_page">
	...
	<div>
		...
		<div class="floatleft">
			...
			<div class="profile_i">
				<a
					onclick="user_interaction('{$user_profile->user_key}','ask_question')">{$languages->ask_question}</a>
			</div>
			<div class="profile_i">
				<a
...
<div class="right_profile_page">
	...
	<div style="width: 48%; float: left;">
		<div id="profile_groups">
			...
			<div id="right-1">
				{foreach from=$groups item=group} <a {if $settings->only_logged_in_users_can_view_group_info
					==
					'yes'}onclick="show_user_notification('{$languages->user_not_logged}')"
					{else}{if
					$no_ajax=='yes'}href="{$settings->site_url}group/{$group->group_name}"
		...
		<div id="profile_blogs">
			...
			<div id="right-1">
				{foreach from=$blogs item=blog} <a
					onclick="show_blog_profile_details('{$blog->id}','{$blog->user_key}')">{$blog->title}</a><br />
