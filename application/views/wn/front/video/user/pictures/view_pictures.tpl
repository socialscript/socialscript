...
<a onclick="subscribe_to_pictures('{$picture_user_key}')">{$languages->subscribe}</a>
{if $pictures_subscribers|count > 0}
...
<div id="galleries_profile">
			<select name="galleries" id="select_picture_galleries_profile"
				class="ui-widget-header select " onchange="change_gallery(this)"> {foreach
				from=$pictures_galleries item=user_gallery}
				<option value="{$user_gallery->id}">{$user_gallery->gallery_name}</option>
				{/foreach}
			</select>
		</div>
		<div id="gallery_pictures">
			{include file="$tpl_dir/user/pictures/gallery_pictures.tpl"}
		</div>
</div>
