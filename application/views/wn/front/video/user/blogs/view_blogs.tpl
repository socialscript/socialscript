...
{if $blog_subscribers|count > 0}
Subscribers:
{foreach from=$blog_subscribers item=subscriber}
{$subscriber.user},
{/foreach}
{/if}
<br />
			<select name="galleries" id="select_blogs_profile"
				class="ui-widget-header select " onchange="change_blog_category()"> {foreach
				from=$user_blog_categories item=blog}
				<option value="{$blog->id}">{$blog->category}</option>
				{/foreach}
			</select>
		</div>
		<div id="blogs">
