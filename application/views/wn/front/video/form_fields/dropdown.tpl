...
<input type="hidden" name="dropdown_{$form_field->id}" value="{$form_field->id}">
<select name="{$form_field->field_name}" id="{$form_field->field_name}"
	...
	<option value="{$options.field_name}" {if $options.checked == '1'}selected="selected"{/if}>{$options.name}</option>
	{/foreach}
</select>
<br />