...
<div id="left_bottom_box">
	<div>
		...
		By: <a onclick="view_profile('{$blog->user_key}')">{$blog->username}</a> {$blog->timestamp|date_format:$settings->date_format}
		</div>
		</div>
	...
	<div>
		...
		<div>
			...
			<div class="events_left ui-widget-header">
				<a onclick="show_event_details('{$event->id}','{$event->user_key}')">{$event->title}<br />{$event->location},{$event->event_date}</a>
		<br />
		<div class="events_info ">
		By: <a onclick="view_profile('{$event->user_key}')">{$event->username}</a> {$event->timestamp|date_format:$settings->date_format}
		</div>
		</div>
	...
	<div class="group   ui-widget-header">
		...
		{$languages->creator}: <a onclick="view_profile('{$group->user_key}')">{$group->username}</a> {$group->timestamp|date_format:$settings->date_format}
		</div>
		</div>
