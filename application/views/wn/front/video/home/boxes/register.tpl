...
<div class="register hidden" id="register">
	<h3>
		<a href="#">{$languages->register_title}</a>
	</h3>
 ...
 ui-widget-header input"
			...
			<br> <label for="email">{$languages->register_email}</label> <input
				type="text" id="email"
 ...
 ui-widget-header input"
			...
			<select class="ui-widget-header select " name="year"> {foreach
				from=$years_dropdown item=validator}
				<option value="{$validator}">{$validator}</option>{/foreach}
			</select> <select class="ui-widget-header select" name="month">
				{foreach from=$months_dropdown item=validator}
				<option value="{$validator}">{$validator}</option>{/foreach}
			</select> <select class="ui-widget-header select" name="day">
				{foreach from=$days_dropdown item=validator}
				<option value="{$validator}">{$validator}</option>{/foreach}
			</select> <br />{$languages->register_gender}:
			<ul class="imageless-css-3-form-elements">
				<label><input type="radio" name="gender" id="male" value="male" checked="checked"><span>{$languages->register_gender_male}</span></label>
			</ul>
			<ul class="imageless-css-3-form-elements">
			...
			<br> {if $settings->show_countries_dropdown_on_register == 'yes'} <label
				...
				name="country" class="ui-widget-header select "> {foreach
