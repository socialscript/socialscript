	...
	<div class="middle_min_height">
		<form method="get" action="index.php?route=users&action=search">
			<input type="hidden" name="route" value="users"> <input type="hidden"
				...
				id="username" class="ui-widget-header input">{$languages->search_username}
...
-->
			{if $settings->show_countries_dropdown_on_register == 'yes'} <label
				...
				name="country" id="search_country" class="ui-widget-header select">
				...
				{/foreach}
			</select> {/if} <br /> {$languages->search_order_by} <select
				name="order_by" id="order_by" class="ui-widget-header select">
				...
				value="Search" class="ui-widget-header input">
