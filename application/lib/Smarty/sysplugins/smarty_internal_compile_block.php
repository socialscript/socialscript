...
class Smarty_Internal_Compile_Block extends Smarty_Internal_CompileBase {
	...
	public static function saveBlockData($block_content, $block_tag, $template, $filepath) {
		...
		if(0 == preg_match("!({$_ldl}{$al}block\s+)(name=)?(\w+|'.*'|\".*\")(\s*?)?((append|prepend|nocache)?(\s*)?(hide)?)?(\s*{$_rdl})!", $block_tag, $_match)) {
			...
			$_name = trim($_match[3], '\'"');
			if($_match[8] != 'hide' || isset($template->block_data[$_name])) { // replace
			                                                                   // {$smarty.block.child}
			                                                                   // do
			                                                                   // we
			                                                                   // have
...
class Smarty_Internal_Compile_Blockclose extends Smarty_Internal_CompileBase {
	...
	public function compile($args, $compiler) {
		...
		if(isset($compiler->template->block_data[$_name]) && ! isset($compiler->template->block_data[$_name]['compiled'])) {
			// restore to status before {block} tag as new subtemplate code of
			// parent {block} is not needed
			// TODO: Below code was disabled in 3.1.8 because of problems with
			// {include} in nested {block} tags in child templates
			// combined with append/prepend or $smarty.block.parent
			// For later versions it should be checked under which conditions it
			// could run for optimisation
			//
