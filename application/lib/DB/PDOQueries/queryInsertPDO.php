...
class queryInsertPDO {
					...
					if($this->Prepared === FALSE) {
						...
						try {
							...
							if($this->queryResult === FALSE) {
								...
								throw new customPDOException($this->Prepared->errorInfo(), 'Unable to execute prepared query<br /> query: ' . $this->uQuery);
							}
	...
	private function showQuery() {
		...
		$this->queryToPrint = '--------------------------------------------  QUERY-----------------------------------' . '<br>' . '<span style="color: red">' . $this->uQuery . '</span>' . '<br>' . '--------------------------------------------  QUERY-----------------------------------';
	...
	private function checkValidQuery() {
		...
		if(strtoupper(substr($trimmed, 0, 6)) != 'INSERT') {
			// print 'not ok';
		}
	}
}
